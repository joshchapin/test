...
class Job {
    ...
    public Job(Button myButton, int baseGoldValue) {
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button btnT1 = findViewById(R.id.btnAdd1);
        levelOne = new Job(btnT1, 1);
        levelOne.setActive();

        Button btnT2 = findViewById(R.id.btnAdd10);
        levelTwo = new Job(btnT2, 10);
        //levelTwo.setIsNext();
    ...
    public void buyLevel(View view){
        int buyAmount = 1;

        if(view.getId() == R.id.btnAdd1){
            if(player.canAfford(levelOne.getCostOfLevels(1))){
                player.spendFunds(levelOne.getCostOfLevels(1));
                levelOne.buyLevel(1);
                levelTwo.setActive();
            }
        }
        else if(view.getId() == R.id.btnAdd10){
            if(player.canAfford(levelTwo.getCostOfLevels(1))) {
                player.spendFunds(levelTwo.getCostOfLevels(1));
                levelTwo.buyLevel(1);
            }
        }
